...
def draw_with_blend_and_clicks(img, mask=None, alpha=0.6, prompt_list=None, pos_color=(0, 255, 0),
                               neg_color=(255, 0, 0), radius=4):
    result = img.copy()
    clicks_list = prompt_list
    if mask is not None:
...
def draw_with_blend_and_boxes(img, mask=None, alpha=0.6, prompt_list=None, pos_color=(0, 255, 0),
                             neg_color=(255, 0, 0), thickness=2):
    result = img.copy()
    boxes_list = prompt_list
    if mask is not None:
        palette = get_palette(np.max(mask) + 1)
        rgb_mask = palette[mask.astype(np.uint8)]
        ...
        if boxes_list is not None and len(boxes_list) > 0:
            ...
            for box in boxes_list:
                ...
                result = cv2.rectangle(result, (x1, y1), (x2, y2),
                                     pos_color, thickness)


    return result
def draw_with_blend_and_masks(img, alpha=0.6, prompt_list=None, pos_color=(0, 255, 0),
                                 neg_color=(255, 0, 0)):
    result = img.copy()
    masks_list = prompt_list
