...
class ZoomIn(BaseTransform):
    ...
    def inv_transform(self, prob_map, click, side_effects=True):

        if self._object_roi is None or self.fast_path:
            if side_effects:
                self._prev_probs = prob_map.cpu().numpy()
            return prob_map, click
        ...
        click = self._inv_transform_clicks(click)

        return new_prob_map, click
    ...
    def _inv_transform_clicks(self, click):
        if self._object_roi is None:
            return click
        rmin, rmax, cmin, cmax = self._object_roi
        crop_height, crop_width = self._roi_image.shape[2:]

        new_r = (click.coords[0] * (rmax - rmin + 1) / crop_height) + rmin
        new_c = (click.coords[1] * (cmax - cmin + 1) / crop_width) + cmin
        return click.copy(coords=(new_r, new_c))

class ZoomInWithBoxes(ZoomIn):

    @override
    def transform(self, image_nd, boxes_lists):
        assert image_nd.shape[0] == 1 and len(boxes_lists) == 1
        self.image_changed = False

        boxes_list = boxes_lists[0]
        if len(boxes_list) == 0:
            self.fast_path = True
            return image_nd, boxes_lists
        ...
        if self._prev_probs is not None:
            current_pred_mask = (self._prev_probs > self.prob_thresh)[0, 0]
            if current_pred_mask.sum() > 0:
                current_object_roi = get_object_roi(current_pred_mask, [],
                                                self.expansion_ratio, self.min_crop_size)
        ...
        tboxes_lists = [self._transform_boxes(boxes_list)]
        self.fast_path = False
        return self._roi_image.to(image_nd.device), tboxes_lists

    def _transform_boxes(self, boxes_list):
        if self._object_roi is None:
            return boxes_list
        ...
        for box in boxes_list:
            ...
            new_x = crop_width * (box[0] - cmin) / (cmax - cmin + 1)
            # Transform y coordinate
            new_y = crop_height * (box[1] - rmin) / (rmax - rmin + 1)
            # Transform width
            new_w = crop_width * box[2] / (cmax - cmin + 1)
            # Transform height
            new_h = crop_height * box[3] / (rmax - rmin + 1)

            transformed_boxes.append([new_x, new_y, new_w, new_h])
        return transformed_boxes

    def _inv_transform_bboxes(self, boxes_list):
        if self._object_roi is None:
            return boxes_list
        ...
        transformed_boxes = []
        for box in boxes_list:
            # Convert from crop coordinates back to original image coordinates
            new_x = box[0] * (cmax - cmin + 1) / crop_width + cmin
            new_y = box[1] * (rmax - rmin + 1) / crop_height + rmin
            new_w = box[2] * (cmax - cmin + 1) / crop_width
            new_h = box[3] * (rmax - rmin + 1) / crop_height
