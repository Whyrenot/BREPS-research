...
def get_predictor(net, brs_mode, device,
    ...
    match prompt_modality:
        case "click":
            PredictorModule = BasePredictor
        case "bbox":
            PredictorModule = BaseBboxPredictor
        case _:
            raise NotImplementedError(f"Failed to match modality for {prompt_modality}")

    if brs_mode == 'NoBRS':
        if predictor_params is not None:
            predictor_params_.update(predictor_params)
        predictor = PredictorModule(net, device, zoom_in=zoom_in, with_flip=with_flip, model_name=model_name, **predictor_params_)
    else:
