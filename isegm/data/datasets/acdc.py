...
class ACDCDataset(ISDataset):
    def __init__(self, dataset_path, split='test', **kwargs):
        ...
        for f in self.images_dir.glob('*.npy'):
            name = f.stem
            mask_path = self.masks_dir / f"{name}_gt.npy"
            if mask_path.exists():
    ...
    def get_sample(self, index) -> DSample:
        ...
        mask = np.load(self.masks_dir / f"{name}_gt.npy").astype(np.int32)
        ...
        sample.imname = name
        sample.image_name = f"{name}.npy"
        sample.mask_name = f"{name}_gt.npy"
        return sample
